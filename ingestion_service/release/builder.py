...
class ProductionReleaseBuilder:
    ...
    def build(self, identity: ReleaseIdentity, stage_path: Path) -> None:
        ...
        try:
            ...
            dist_catalog = dist_dir / "catalog.json"
            if dist_catalog.is_symlink() or not dist_catalog.is_file():
                raise ReleaseBuildError(
                    "Frontend build did not produce the authoritative dist/catalog.json"
                )
            ...
            if source_books.is_dir():
                shutil.copytree(source_books, resolve_contained_path(stage, "books"), symlinks=False, dirs_exist_ok=True)
            # catalog.json is generated by the frontend artifact generator and
            # is the authoritative runtime catalog. Never reconstruct or
            # overwrite it after npm has produced dist.
