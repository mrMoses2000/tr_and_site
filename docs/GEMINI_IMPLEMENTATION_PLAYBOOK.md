...
Полные PDF не входят в Git. Перед P3/P14 исполнитель ищет разрешённые owner-provided sources в `/srv/logos/sources/incoming`, `storage/inbox` и `storage/processed`, вычисляет SHA-256 самостоятельно и сопоставляет только с таблицей выше. Filename, title metadata и похожее количество страниц не подтверждают идентичность. Отсутствующий hash фиксируется как `MISSING_SOURCE`; скачивать или подменять книгу из интернета запрещено. После импорта источник копируется в immutable content-addressed storage, а исходный owner-файл не удаляется.
